...
export function StatusBadge({ status }) {
  const c = STATUS_COLORS[status] || { bg:'rgba(100,116,139,.2)', text:'#94a3b8', dot:'#64748b' };
  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:6, background:c.bg, color:c.text, padding:'3px 10px', borderRadius:999, fontSize:12, fontWeight:500 }}>
      <span style={{ width:6, height:6, borderRadius:'50%', background:c.dot, animation: ['Submitted','Pending ISO Closure','Pending Admin Approval'].includes(status) ? 'pulse-dot 1.5s ease infinite' : 'none' }} />
      {status}
    </span>
  );
}
